...
# The connection is stored and resued instead of creating a new connection if 'get_db' is called a second time in the same request.
...
# 'open_resource()' opens a file relative to the 'flaskr' package, which is useful since you won't necessarily know where that location is when 
# deploying the application later. 'get_db' returns a database connection, which is used to execute the commnands read from the file.
